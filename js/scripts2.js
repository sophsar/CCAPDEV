...
// Function to arrange restaurants alphabetically
function arrangeAlphabetically() {
    ...
    // Clear current content of restaurants container
    restaurantsContainer.innerHTML = '';
    
    // Append sorted restaurants to the container
    restaurants.forEach(restaurant => {
        restaurantsContainer.appendChild(restaurant);
    });
}

// Event listener for alphabetical button click
const alphabeticalButton = document.querySelector('.alphabetical');
alphabeticalButton.addEventListener('click', function() {
    // Toggle active class on button
    this.classList.toggle('active');
    
    // If button is active, arrange alphabetically, else reset order
